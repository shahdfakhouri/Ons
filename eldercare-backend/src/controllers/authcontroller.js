...
exports.signup = async (req, res) => {
  ...
  try {
    // 🧠 pick the correct email column for each role
    const emailColumn =
      role.toLowerCase() === "retirement_home"
        ? "contact_email"
        : "email";

    // check if user already exists in the right column
...
exports.signin = (req, res) => {
  ...
  // 🧠 pick correct email column based on role
  const emailColumn =
    role.toLowerCase() === "retirement_home"
      ? "contact_email"
      : "email";
  ...
  db.query(`SELECT * FROM ${table} WHERE ${emailColumn} = ?`, [email], async (err, result) => {
    ...
    // 🚫 check approval for restricted roles
    ...
    // 🧩 identify primary key field per role
    const idField =
      role === "admin"
        ? "admin_id"
        : role === "caregiver"
